...
Apply the `impact-grid-editorial` Style Lock. Create an Image2-first full-slide image with a high-end consulting editorial report style: strict black-line grid, warm-white institutional background, deep teal accents, documentary low-saturation photography, oversized condensed typography, hard rectangular modules, tiny report metadata, and restrained information density. Chinese-first visible text. Keep every slide consistent with the same grid, line weight, palette, footer behavior, and rectangular container system. Do not add any black border or full-page outline on the outer canvas edge.
